...
- 每张卡片 4 行，印在 四线三格 格子上：
  - 2 行 Hershey Futural 单笔画字母描红（实线，非虚线）；**首份纯黑 `#000000` 参考 + 若干浅蓝 `#b8d9ee` 副本铺满整行**，小朋友沿浅色笔画描红
  - 2 行空白练习
  - 四线格通过**双 SVG 架构**（v1.3.0+）铺到容器最右侧；副本间距从 v1.4.0 起改为**动态均分**（justify 风格，gap ≥ 60 SVG 单位 ≈ 10mm 打印），每行右边缘贴近视觉边界，再没有参差不齐的尾部留白
...
- **PDF 生成后自动送默认打印机**（Linux/macOS 走 CUPS `lp`）。无 `lp` 或无可用打印机自动跳过，只在 stderr 通知。用 `--no-print` 或 `TRACING_CARDS_AUTO_PRINT=0` 禁用；用 `TRACING_CARDS_PRINTER=<name>` 指定打印机。
